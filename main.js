...
function saveDatabaseToDisk() {
  ...
  .then(res => {
    if (res.status === 'success') {
      ...
      showToast('❌ خطا در ذخیره روی دیسک: ' + res.message, 'error');
    }
  })
  .catch(err => {
...
function openPanel(eventId) {
  ...
  // Badge
  panelEraBadge.textContent = era.title;
...
function saveNewRef() {
  ...
  if (!title || !activeEventId) { showToast('عنوان منبع الزامی است', 'error'); return; }
...
function renderEditList(filter = '') {
  ...
  if (events.length === 0) {
    editList.innerHTML = '<p style="color:var(--text-muted);font-size:0.85rem;padding:12px 0">رویدادی یافت نشد.</p>';
    return;
  }
...
function setupEventListeners() {
  ...
  $('ai-ingest-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showToast('❌ خطا در جذب داده: ' + err.message, 'error');
      spinner.style.display = 'none';
    } finally {
